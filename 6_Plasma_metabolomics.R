...
data<-data[, c(1:5, grep("Plasma.metabolomics", names(data)))]
...
for(h in 1:length(hits)){
	...
	gdat$y<-data[,grep(hits[h], names(data))]
...
setwd(wd)
write.table(results, file="R/Plasma_metab_effects.csv", sep=",", row.names=F, col.names=names(results))
