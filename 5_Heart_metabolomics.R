...
data<-data[, c(1:5, grep("Heart.metabolomics", names(data)))]
...
for(i in 6:length(data)){
	...
	# log variables - turns coefs in lnRR - NOTE data are already log2 transformed at normalisation
	#dat_i$y<-log(dat_i$y)
...
# The hit for an interaction and main effects is for Thiamine (HILIC) - plot that out
d<-ggplot(data, aes(x=Diet.group, y=(Heart.metabolomics_HILIC_Thiamine), col=Diet.group, fill=Diet.group)) +
	geom_violin() + geom_point(col="black") + xlab("Diet") + ylab("Log(L Thiamine) [HILIC]") + labs(subtitle="D. (Main Effects and Interaction)") + 
	theme_bw() + theme(legend.position="none")
d

# Also have main effects of fat on Homocysteate..HCA. (AMIDE) and Oxaloacetate (AMIDE)
e<-ggplot(data, aes(x=Diet.group, y=(Heart.metabolomics_AMIDE_Homocysteate..HCA.), col=Diet.group, fill=Diet.group)) +
	geom_violin() + geom_point(col="black") + xlab("Diet") + ylab("Log(Homocysteate HCA) [AMIDE]") + labs(subtitle="E. (Main Effect of Fat)") + 
	theme_bw() + theme(legend.position="none")
e

f<-ggplot(data, aes(x=Diet.group, y=(Heart.metabolomics_AMIDE_Oxaloacetate), col=Diet.group, fill=Diet.group)) +
	geom_violin() + geom_point(col="black") + xlab("Diet") + ylab("Log(Oxaloacetate) [AMIDE]") + labs(subtitle="F. (Main Effect of Fat)") + 
	theme_bw() + theme(legend.position="none")
f
...
setwd(wd)
write.table(results, file="R/Heart_metab_effects.csv", sep=",", row.names=F, col.names=names(results))
